...
base_dir <- here("3_intensity/intensity_index/input")
...
for (target in targets) {
  ...
  cohacoca_file_path <- file.path(base_dir, paste0("amod.", target, ".year.count.cohacoca.csv"))
  input_paths[[paste0(target, ".cohacoca")]] <- cohacoca_file_path
  
  psych_file_path <- file.path(base_dir, paste0("amod.", target, ".year.count.psych.csv"))
...
output_path <- here("3_intensity/intensity_index/output/amod.rel_count")
